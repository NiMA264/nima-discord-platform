function createApiClient({ baseUrl }) {
    return {
        ...
        async getWorkflowSuggestions(accessToken, guildId, workspaceId = '') {
            const workspaceQuery = workspaceId ? `&workspaceId=${encodeURIComponent(workspaceId)}` : '';
            const response = await fetch(`${baseUrl}/api/analytics/suggestions?guildId=${encodeURIComponent(guildId || '')}${workspaceQuery}`, {
                headers: { authorization: `Bearer ${accessToken}` }
            });

            if (!response.ok) {
                throw new Error(`Workflow suggestions API request failed: ${response.status}`);
            }
